...
function writeLocalDrafts(storageKey: string, drafts: Record<string, string>) {
  ...
  try {
    ...
    // Private browsing and full storage quotas can reject localStorage writes.
  }
}

function writeLocalJson(storageKey: string, value: unknown) {
  if (!storageKey) return;
  try {
    localStorage.setItem(storageKey, JSON.stringify(value));
  } catch {
    // The app must keep working when device storage is unavailable or full.
  }
}

function removeLocalValue(storageKey: string) {
  if (!storageKey) return;
  try {
    localStorage.removeItem(storageKey);
  } catch {
    // Storage cleanup is best-effort and must never block account switching.
  }
}
...
export function KuzensApp() {
  ...
  const [syncState, setSyncState] = useState<"online" | "reconnecting" | "offline">(
  ...
  const activeDirectConversationIdRef = useRef(activeDirectConversationId);
  ...
  const draftStorageName = profile?.id ? `kuzens-drafts:${profile.id}` : "";
  const memberVolumesStorageName = profile?.id ? `kuzens-member-volumes:${profile.id}` : "";
  const favoriteChannelsStorageName = profile?.id ? `kuzens-favorite-channels:${profile.id}` : "";
  ...
  useEffect(() => {
    const wentOffline = () => setSyncState("offline");
    const cameOnline = () => {
  ...
  useEffect(() => {
    ...
    async function syncMessages(initial = false) {
      ...
      try {
        ...
        if (!cancelled) setSyncState(navigator.onLine ? "reconnecting" : "offline");
  ...
  useEffect(() => {
    ...
    writeLocalJson("kuzens-preferences", preferences);
  }, [preferences]);
  ...
  useEffect(() => {
    memberVolumesReady.current = false;
    if (!memberVolumesStorageName) {
      window.queueMicrotask(() => setMemberVolumes({}));
      return;
    }
    window.queueMicrotask(() => {
      try {
        const saved = JSON.parse(localStorage.getItem(memberVolumesStorageName) || "{}");
        ...
        removeLocalValue("kuzens-member-volumes");
      }
    });
  }, [memberVolumesStorageName]);

  useEffect(() => {
    if (!memberVolumesStorageName || !memberVolumesReady.current) return;
    writeLocalJson(memberVolumesStorageName, memberVolumes);
  }, [memberVolumes, memberVolumesStorageName]);

  useEffect(() => {
    favoriteChannelsReady.current = false;
    if (!favoriteChannelsStorageName) {
      window.queueMicrotask(() => setFavoriteChannelIds(new Set()));
      return;
    }
    window.queueMicrotask(() => {
      try {
        const saved = JSON.parse(localStorage.getItem(favoriteChannelsStorageName) || "[]");
        ...
        removeLocalValue("kuzens-favorite-channels");
      }
    });
  }, [favoriteChannelsStorageName]);

  useEffect(() => {
    if (!favoriteChannelsStorageName || !favoriteChannelsReady.current) return;
    writeLocalJson(
      favoriteChannelsStorageName,
      Array.from(favoriteChannelIds).slice(0, 100),
    );
  }, [favoriteChannelIds, favoriteChannelsStorageName]);

  useEffect(() => {
    if (!draftStorageName) {
      window.queueMicrotask(() => setSavedDraftKeys(new Set()));
      return;
    }
    const saved = readLocalDrafts(draftStorageName);
    window.queueMicrotask(() => setSavedDraftKeys(new Set(Object.keys(saved))));
    // Older builds used one device-wide bucket. Do not expose those drafts to
    // another account that later signs in on the same computer.
    removeLocalValue("kuzens-drafts");
  }, [draftStorageName]);
  ...
  }, [channelDraftKey, draftStorageName]);

  useEffect(() => {
    if (!draftStorageName || draftLoadedKey !== channelDraftKey) return;
    const saved = readLocalDrafts(draftStorageName);
    if (draft) saved[channelDraftKey] = draft;
    else delete saved[channelDraftKey];
    writeLocalDrafts(draftStorageName, saved);
    window.queueMicrotask(() => {
      setSavedDraftKeys((current) => {
        const next = new Set(current);
        if (draft) next.add(channelDraftKey);
        else next.delete(channelDraftKey);
        return stringSetsEqual(current, next) ? current : next;
      });
    });
  }, [channelDraftKey, draft, draftLoadedKey, draftStorageName]);
  ...
  }, [directDraftKey, draftStorageName]);

  useEffect(() => {
    if (!draftStorageName || directDraftLoadedKey !== directDraftKey) return;
    const saved = readLocalDrafts(draftStorageName);
    ...
    window.queueMicrotask(() => {
      setSavedDraftKeys((current) => {
        const next = new Set(current);
        if (directDraft) next.add(directDraftKey);
        else next.delete(directDraftKey);
        return stringSetsEqual(current, next) ? current : next;
      });
    });
  }, [directDraft, directDraftKey, directDraftLoadedKey, draftStorageName]);
  ...
  }, [threadDraftKey, draftStorageName]);

  useEffect(() => {
    if (!draftStorageName || threadDraftLoadedKey !== threadDraftKey) return;
    const saved = readLocalDrafts(draftStorageName);
    ...
    window.queueMicrotask(() => {
      setSavedDraftKeys((current) => {
        const next = new Set(current);
        if (threadDraft) next.add(threadDraftKey);
        else next.delete(threadDraftKey);
        return stringSetsEqual(current, next) ? current : next;
      });
    });
  }, [threadDraft, threadDraftKey, threadDraftLoadedKey, draftStorageName]);
  ...
  async function sendMessage(event: FormEvent) {
    ...
    const submittedChannelId = activeChannel;
    const submittedDraftKey = channelDraftKey;
    ...
    const optimistic: ChatMessage = {
      ...
      content,
      replyToId: submittedReply?.id || null,
    ...
    try {
      ...
      if (failedFiles.length) {
        setToast({
          text: `Mesaj gönderildi; ${failedFiles.length} dosya yüklenemedi. Dosyalar tekrar denemen için hazır tutuldu.`,
          tone: "danger",
        });
      }
      ...
      if (!persistedMessage) {
        ...
        const saved = readLocalDrafts(draftStorageName);
        saved[submittedDraftKey] = content;
        writeLocalDrafts(draftStorageName, saved);
        setSavedDraftKeys((current) => new Set(current).add(submittedDraftKey));
        if (activeChannelRef.current === submittedChannelId) {
          setDraft(content);
          setReplyingTo(submittedReply);
        }
      }
      setToast({
        text: persistedMessage
          ? "Mesaj gönderildi ancak ek işlemlerden biri tamamlanamadı. Mesajın kaybolmadı."
          : error instanceof Error
            ? `${error.message} Taslağın korundu.`
  ...
  async function sendDirectMessage(event: FormEvent<HTMLFormElement>) {
    ...
    const submittedConversationId = activeDirectConversation.id;
    const submittedDraftKey = `direct:${submittedConversationId}`;
    const optimistic: DirectMessage = {
      id: `local-direct-${crypto.randomUUID()}`,
    ...
    try {
      ...
      writeLocalDrafts(draftStorageName, saved);
      setSavedDraftKeys((current) => new Set(current).add(submittedDraftKey));
      if (activeDirectConversationIdRef.current === submittedConversationId) {
        setDirectDraft(content);
      }
      setToast({
        text: error instanceof Error
          ? `${error.message} Taslağın korundu.`
          : "Özel mesaj gönderilemedi. Taslağın korundu.",
        tone: "danger",
      });
    }
  }
  ...
  function channelHasDraft(channelId: string) {
    const key = `channel:${activeServerId}:${channelId}`;
    return key === channelDraftKey ? Boolean(draft.trim()) : savedDraftKeys.has(key);
  }

  function directConversationHasDraft(conversationId: string) {
    const key = `direct:${conversationId}`;
    return key === directDraftKey ? Boolean(directDraft.trim()) : savedDraftKeys.has(key);
  }
    ...
    return (
      ...
      >
        ...
        >
          ...
          {channel.kind !== "voice" && channelHasDraft(channel.id) && (
            <em className="draft-badge">Taslak</em>
          )}
          ...
          <div className="channel-section">
            ...
            {uncategorizedTextChannels.map((channel) => (
              ...
              >
                ...
                <span>{channel.name}</span>
                {channelHasDraft(channel.id) && <em className="draft-badge">Taslak</em>}
                {Boolean(channel.mentionCount && channel.notificationLevel !== "none") && (
                  <em className="mention-badge">{Math.min(99, channel.mentionCount || 0)}</em>
                )}
        ...
        <header className="chat-header">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (navigator.onLine) setSyncGeneration((current) => current + 1);
            }}
            title={syncState === "online" ? "Mesajlar canlı eşitleniyor" : "Bağlantıyı yeniden dene"}
            aria-label={
              syncState === "online"
                ? "Mesaj eşitleme bağlantısı canlı"
                : syncState === "offline"
                  ? "İnternet bağlantısı yok, yeniden dene"
                  : "Mesaj bağlantısı yeniden kuruluyor"
            }
          >
            <i />
            <span>{syncState === "online" ? "Canlı" : syncState === "offline" ? "Çevrimdışı" : "Bağlanıyor"}</span>
          </button>
                  ...
                  directConversations.map((conversation) => (
                    ...
                    >
                      ...
                      <span>
                        ...
                        <small>
                          {directConversationHasDraft(conversation.id) ? (
                            <><b className="direct-draft-label">Taslak</b> · {conversation.lastMessage || "Mesajın hazır"}</>
          ...
          >
            ...
            <section className="preference-section install-section">
              ...
              <div className="install-actions">
                <button type="button" className={installedApp ? "installed" : ""} onClick={() => void installKuzens()}><span>{installedApp ? "✓" : "⬇"}</span><strong>{installedApp ? "Kuzens kurulu" : "Kuzens’i güvenli yükle"}</strong><small>{installedApp ? "Bağımsız uygulama modunda çalışıyor" : "EXE indirmeden, SmartScreen uyarısı olmadan kur"}</small></button>
